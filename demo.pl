...
#this will go through the registry file and look for the target query, for now it is done by getting input from stdin
#for ease of testing purposes.
#todo: alter it to get the target from command line arguments (@ARGV)
#todo: have the website call this script to with command line arg of the search query and have it return the record(s)
#todo: add in links to the registry file

my($query, $input, $exit); #declare variables
$exit = 0; #is 1 when the program should exit, 0 when it still needs to run
while($exit != 1) #while we dont want to exit
{
   print "What would you like to search for? enter -1 to quit ";  #get the query, will be from @ARGV in future
   $query = <STDIN>; #gets input from stdin
   chomp($query); #removes the new line character
   if($query eq "-1") #if it is the exit condition
   {
   ...
   while(($exit == 0) and (eof(INPUT) != 1)) #while we dont want to exit and not at the end of file
   {
      ...
      if($input =~ /$query/i) #if the line contains the given query
      {
